...
class mainBG():
    ...
    def open_windows(self,name,pos):
        ...
        self.set_eva(self.newWindow.get_text(),self.newWindow.get_text1(),self.newWindow.get_line_text(),self.newWindow.get_line_text1(),self.newWindow.get_filename(),pos)
        self.set_bar()
        
    def set_eva(self,text,text0,line_text,line_text0,file_name,pos):
        self.eva_windows = EvaAction(self.tagData,pos,self.root, self.eva_place,file_name, text, text0, line_text,line_text0, self.ws, int(2 * self.hs / 8),self.bar_buttom,self.bar_right)
